...
export class RabbitMQPublisher implements EventPublisherPort, OnModuleInit {
    ...
    async onModuleInit() {
        try {
            ...
            await this.channel.assertExchange('quotation', 'topic', { durable: true });
            console.log('✅ Quotation Service connected to RabbitMQ');
        } catch (error) {
            console.error('❌ Quotation Service RabbitMQ connection failed', error);
        }
    }
